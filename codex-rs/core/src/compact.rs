...
mod tests {
    ...
    #[tokio::test]
    async fn build_local_compact_result_uses_frozen_history_snapshot() {
        let (session, turn_context) = crate::codex::make_session_and_context().await;
        session
            .replace_history(
                vec![ResponseItem::Message {
                    id: Some("live-user".to_string()),
                    role: "user".to_string(),
                    content: vec![ContentItem::InputText {
            ...
            .await;
        let frozen_history = vec![
            ResponseItem::Message {
                id: Some("frozen-user".to_string()),
                role: "user".to_string(),
                content: vec![ContentItem::InputText {
                    text: "frozen history".to_string(),
                }],
                end_turn: None,
                phase: None,
            },
        ...
        let result = build_local_compact_result(
            &session,
            &turn_context,
            &frozen_history,
            InitialContextInjection::DoNotInject,
        ...
        let expected_history = build_compacted_history(
            Vec::new(),
            &["frozen history".to_string()],
            &expected_summary,
        );

        assert_eq!(expected_history, result.replacement_history);
        assert_eq!(
            Some(expected_history),
            result.compacted_item.replacement_history
        );
        assert_eq!(expected_summary, result.compacted_item.message);
