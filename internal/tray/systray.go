...
import (
	...
	"github.com/gabewillen/codextra/internal/tray/internal"
)
...
func New() (*SystemTray, error) {
	...
	if err := platform.Create(); err != nil {
		return nil, fmt.Errorf("failed to create platform tray: %w", err)
	}
	return t, nil
}
...
// RunUntil blocks the calling goroutine, pumping the platform message loop.
// Returns when Quit() is called or the stop channel is closed.
func (t *SystemTray) RunUntil(stop <-chan struct{}) error {
