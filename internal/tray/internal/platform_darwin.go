...
import (
	...
	"github.com/gabewillen/codextra/internal/tray/internal/darwin"
)
...
var darwinSels struct {
	...
	setAutoenablesItems         darwin.SEL // setAutoenablesItems:
...
var darwinTrayRunForever = make(chan struct{})
...
func initDarwinSels() {
	darwinSels.once.Do(func() {
		...
		darwinSels.setAutoenablesItems = darwin.RegisterSelector("setAutoenablesItems:")
...
func (t *darwinTray) buildNSMenu(title string, menu *Menu, counter *int) darwin.ID {
	...
	// Disable Cocoa's auto-enable so explicit Disabled items stay greyed.
	nsMenu.SendBool(darwinSels.setAutoenablesItems, false)
	...
	for _, item := range menu.Items {
		...
		default:
			...
			// Disabled items get no action so a click never lands on the target.
			action := uintptr(menuClickedSel)
			if item.Disabled {
				action = 0
			}
			nsItem = darwin.MsgSend3Ptr(nsItem, darwinSels.initWithTitleActionKeyEquiv,
			...
			// Apply enabled/disabled state explicitly because the parent menu
			// has setAutoenablesItems:NO.
...
// Run blocks the calling goroutine, running a Cocoa-compatible event loop.
func (t *darwinTray) Run() error {
	return t.RunUntil(nil)
}

// RunUntil blocks the calling goroutine, running a Cocoa-compatible event loop
// until the provided stop channel is closed.
func (t *darwinTray) RunUntil(stop <-chan struct{}) error {
	if stop == nil {
		stop = darwinTrayRunForever
	}
	...
	// Run a local event pump. `NSApp run` is not used here because it can
	// panic when this package is used from a non-primary thread.
	mode := darwin.NewNSString("kCFRunLoopDefaultMode")
	if mode.IsNil() {
		return errors.New("darwin: failed to create run loop mode string")
	}
	...
	drainMainCalls:
		...
		// Use distantPast so this returns quickly when no event is available.
		event := darwin.MsgSend4Ptr(
			t.nsApp,
			darwinSels.nextEventMatchingMask,
			^uintptr(0), // NSEventMaskAny
			uintptr(darwinClasses.NSDate.Send(darwinSels.distantPast)),
			uintptr(mode),
			1, // dequeue
		)
		if event.IsNil() {
		...
		t.nsApp.SendPtr(darwinSels.sendEvent, event.Ptr())
	}
